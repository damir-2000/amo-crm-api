...
class JsonStep(object):
    ...
    def __repr__(self):
        vals = ",".join("%s=%s" % (key, val) for key, val in self.__dict__.items())
...
def clean_undefined(obj):
    ...
    if isinstance(obj, list):
        return [None if isinstance(item, Undefined) else item for item in obj]
    if isinstance(obj, dict):
        for key in obj:
...
def clean_empty_string(obj):
    ...
    if obj == "":
        return None
    if isinstance(obj, list):
        return [None if item == "" else item for item in obj]
    if isinstance(obj, dict):
        for key in obj:
